...
#     https://colab.research.google.com/drive/1J3g-ZqjI_ZxrR_4Yl8NR8PL3EW7KK6Wq
...
# ✅ Predict on test data using rf2
y_pred_rf2 = rf2.predict(X_test)

# ✅ Save predictions + features for LLM integration
X_test_dict = [dict(enumerate(x)) for x in X_test.to_numpy()]
df_rf_llm = pd.DataFrame({
    "label": y_pred_rf2,
    "features": X_test_dict
})
df_rf_llm.to_csv("llm_input_data_rf.csv", index=False)
print("📄 Saved Random Forest results to llm_input_data_rf.csv")
...
# 1️⃣ **Preprocess Labels** (Convert Categorical to Numeric)
label_encoder = LabelEncoder()
y_train_encoded = label_encoder.fit_transform(y_train)
y_test_encoded = label_encoder.transform(y_test)

# 2️⃣ **Fix Class Imbalance with SMOTE (Data Augmentation)**
smote = SMOTE(sampling_strategy='auto', random_state=42)
X_train_resampled, y_train_resampled = smote.fit_resample(X_train, y_train_encoded)

# 3️⃣ **Compute Class Weights for Imbalance Handling**
class_weights = compute_class_weight("balanced", classes=np.unique(y_train_resampled), y=y_train_resampled)
class_weights_dict = {i: class_weights[i] for i in range(len(class_weights))}

# 4️⃣ **Normalize Features (Standardization)**
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train_resampled)
X_test_scaled = scaler.transform(X_test)

# 5️⃣ **Reshape Data for LSTM (Samples, TimeSteps, Features)**
X_train_reshaped = X_train_scaled.reshape(X_train_scaled.shape[0], 1, X_train_scaled.shape[1])
X_test_reshaped = X_test_scaled.reshape(X_test_scaled.shape[0], 1, X_test_scaled.shape[1])

# 6️⃣ **Define the Updated LSTM Model**
model = Sequential([
    ...
    Dense(len(np.unique(y_train_encoded)), activation='softmax')  # Multi-class classification
])

# 7️⃣ **Compile the Model**
model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

# 8️⃣ **Use Early Stopping to Prevent Overfitting**
early_stopping = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)

# 9️⃣ **Train the Model with Class Weights**
history = model.fit(
    X_train_reshaped, y_train_resampled, validation_data=(X_test_reshaped, y_test_encoded),
    epochs=50, batch_size=32, class_weight=class_weights_dict, callbacks=[early_stopping]
)
...
# 1️⃣1️⃣ **Compute Performance Metrics**
...
# ✅ Use correct number of rows from actual LSTM test set
X_test_original = scaler.inverse_transform(X_test_scaled)  # Optional: revert to original scale
X_test_dict = [dict(enumerate(row)) for row in X_test_original]  # Match 274 rows

df_lstm_llm = pd.DataFrame({
    "label": y_pred,
    "features": X_test_dict
})

df_lstm_llm.to_csv("llm_input_data_lstm.csv", index=False)
print("📄 Saved LSTM results to llm_input_data_lstm.csv")


# 1️⃣2️⃣ **Print Evaluation Metrics**
print(f"Accuracy: {accuracy:.4f}")
print("\nClassification Report:\n", classification_report(y_test_encoded, y_pred, zero_division=1))
print("\nConfusion Matrix (2D List):")
print(conf_matrix.tolist())

# 1️⃣3️⃣ **Plot Training vs Validation Loss**
...
# plt.ylabel("Loss")
# plt.legend()
# plt.title("Training vs Validation Loss to Check Overfitting")
# plt.show()

# 1️⃣4️⃣ **Plot the Confusion Matrix**
